...
#' Take a bootstrap draws based on the observed data
#' and the influence curve.
#' @param data The observed data
#' @param ifc The influence curve
#' @param sims The number of draws desired
#' @param norms the norms underwhich the bootstrap draws chould be compressed.
#'
#' @export
...
single_bootstrap <- function(data, ifc, sims, norms = c("max")){
  ...
  nu_matrix <- matrix(stats::rnorm(sims * num_obs), nrow = num_obs, ncol = sims)
  ...
  for(norm_indx in 1:num_norms){
    ...
    l_p_res[norm_indx] <- as.numeric(stats::quantile(l_p_distr, 0.95) <
