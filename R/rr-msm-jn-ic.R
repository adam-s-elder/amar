...
#' for the estimation (rather than also using other learners).
#'
...
#'
#' Most of the code for this function has been copied and slightly modified
...
rr.msm.jn.ic <- function(obs_data, what = "both", control = NULL) {
  ...
  for(cov_idx in seq_len(length(w_covs))) {
...
my_tmleMSM.jn <- function(Y,A,W,V,T=rep(1,length(Y)), Delta=rep(1, length(Y)), MSM, v=NULL,
                       ...
                       id=seq_len(length(Y)), V_SL=5, inference=TRUE, verbose=FALSE, ret_IC = TRUE) {
  Y[is.na(Y)] <- 0
  n <- length(Y)
