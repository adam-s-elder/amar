...
$$\text{pr}(\varphi(sx + Z) < c_\alpha) \text{ where } Z \sim P_0 \text{ and } c_\alpha = F_{\varphi(Z)}^{-1}(1 - \alpha)$$
is lower than $0.2$.
...
We will discuss specification of the norm in the next section. For more details on the procedure, including why performance measures are good for defining a test statistic, see the paper XXXXXXX.  
...
The choice of $p$ is specified by the `pos_lp_norms` argument. If `pos_lp_norm` is assigned a single value, a non-adaptive version of the test will be performed.  If instead `pos_lp_norm` is assigned multiple arguments an adaptive test will be carried out.  More information can be found in XXXXXX.  For the $\ell_p$ norm, it is possible to set $p = \infty$.  To make this specification in R, include `"max"` in the vector of values assigned to `pos_lp_norm`.
...
## Controling the output of `mv_pn_test`

The last two control arguments determine the output of the `mv_pn_test` function.  

### more_info

The first of these arguments is `more_info`.  If this argument is takes value `FALSE`, then minimum amount of information will be shown (just the p-value).  If this argument is instead `TRUE`, then more information will be displayed, including:
...
The second argument, `ret_cov_mat` if set to `TRUE` (along with `more_info`) will have `var_mat` returned which is the empirical second moment of the IC (equal asymptotically to the variance estimator).  However, this matrix can be quite large for larger dimensions, which is why there is a separate control for this option.
